...
- (void)show
{
    ...
    self.titleLabel.text = LocalizedString(@"String_time_limit");
	
	self.subTitleLabel.center = CGPointMake([UIScreen uu_screenWidth]/2,[UIScreen uu_screenHeight]/2 + 30);
	self.subTitleLabel.text =LocalizedString(@"String_slide_up_to_cancel");
...
- (void)startAnimation
{
	...
	self.centerLabel.textColor = second >= 60.f ? [UIColor redColor] : [UIColor yellowColor];
    self.centerLabel.text = [NSString stringWithFormat:@"%.1f",second+
                             0.1];
}
